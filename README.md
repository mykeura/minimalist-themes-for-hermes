# Minimalist Themes for Hermes Desktop

Give Hermes Desktop a warmer, calmer look with 18 balanced palettes.

**Version 1.2.5 — catalog preparation; not yet available in the curated catalog.**

![Minimalist Themes for Hermes Desktop](images/hermes-cover.png)

## Per-profile colors

The new version of "Minimalist Themes for Hermes" adds per-profile colors: set a different palette for each profile, so you always know which agent you are talking to.

![Per-profile theme colors](demo/demo-1.gif)

This feature ships turned off by default: in the plugin settings every profile reads "Default", which follows the parent (global) theme.

But click each profile's tab and you can switch the theme for that specific profile — a different one per profile if you like.

![Choosing a theme for each profile](demo/demo-2.gif)

I hope you find this feature useful — and thanks to the folks on Reddit whose interest sparked it.

## Install after this branch is published

```bash
hermes plugins install mykeura/minimalist-themes-for-hermes
hermes plugins enable minimalist-themes
```

After catalog acceptance, `hermes plugins install minimalist-themes` will work; it is not available by name yet.

## Update or uninstall

```bash
hermes plugins update minimalist-themes
hermes plugins uninstall minimalist-themes
```

`uninstall` is the CLI alias for `remove`.

## Support and license

[My Nous Portal referral link](https://portal.nousresearch.com/r/mykeura) gives new Personal subscribers **$15 off** and gives me a **$10 referral credit**. Optional, but appreciated.

## Theme gallery

|   |   |
| --- | --- |
| ![Beetroot Juice](images/beetroot-juice.png)<br>Beetroot Juice | ![Blackberry Juice](images/blackberry-juice.png)<br>Blackberry Juice |
| ![Coffee With Milk](images/coffee-with-milk.png)<br>Coffee With Milk | ![Cornmeal Porridge](images/cornmeal-porridge.png)<br>Cornmeal Porridge |
| ![Diana Yin](images/diana-yin.png)<br>Diana Yin | ![Grape Juice](images/grape-juice.png)<br>Grape Juice |
| ![Green Tea](images/green-tea.png)<br>Green Tea | ![Hibiscus Tea](images/hibiscus-tea.png)<br>Hibiscus Tea |
| ![Horchata](images/horchata.png)<br>Horchata | ![Mango](images/mango.png)<br>Mango |
| ![Mint](images/mint.png)<br>Mint | ![Nance Juice](images/nance-juice.png)<br>Nance Juice |
| ![Oceans](images/oceans.png)<br>Oceans | ![Orange Juice](images/orange-juice.png)<br>Orange Juice |
| ![Snow Water](images/snow-water.png)<br>Snow Water | ![Turquoise](images/turquoise.png)<br>Turquoise |
| ![Ultramarine](images/ultramarine.png)<br>Ultramarine | ![Yuzu](images/yuzu.png)<br>Yuzu |

MIT — see [LICENSE](LICENSE).
